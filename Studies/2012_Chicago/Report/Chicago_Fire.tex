...
Based on the information collected, several engineering approximations were made. The actual ignition time is unknown.
 
The following subsections will describe the inputs that were used to construct the simulation, including any approximations or assumptions that were made.
...
Ambient Temperature$^*$                                    &  25~$^{\circ}$C (77~$^{\circ}$F)              & --                                    \\ [.1cm]
...
\end{tabular}\par
\footnotesize
$^{*}$ Initial interior temperatures were assumed to be 25~$^{\circ}$C; exterior temperatures were 7~$^{\circ}$C~\cite{NIOSH:Bowyer}.
\normalsize
\end{table}
...
\bottomrule[1.25pt]
\end{tabular}\par
For specific information regarding ventilation areas used in FDS model, refer to Table~\ref{tab:vents}.
\end{table}
...
After the interior door failed, an interior flow path was created from the enclosed porch area into the kitchen of the structure. This resulted in a rapid change in the conditions within the flow path. The combustion gases at elevated temperature and pressure in the enclosed porch were vented by the failure of the door and flowed towards lower pressure areas in the hallways and kitchen. The arrows in Fig.~\ref{fig:flowpath_1} summarize the simulated flow direction of gases on the second floor following the door failure. Red colored arrows indicate that the gases were at an elevated temperature, while the blue arrows indicate that the gases were at temperatures close to ambient.  The arrow that transitions from red to blue indicates that the gas began to cool as it approached the front of the structure. Note that at the second story doorway towards the front of the structure there are two arrows; this is an indication of bi-directional flow. The cool ambient air flowed in at the bottom while the gas at elevated temperature flowed out the top.
