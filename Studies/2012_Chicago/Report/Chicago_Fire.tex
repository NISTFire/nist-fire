...
Wood can be represented by the the following chemical formula, $\C\H_{1.7}\O_{0.74}\N_{0.002}$, with yields of carbon monoxide ($y_{\mathrm{CO}}=0.004 \; \mathrm{kg}/\mathrm{kg}$) and soot ($y_{\mathrm{C}}=0.015 \; \mathrm{kg}/\mathrm{kg}$) per kg of fuel consumed found from the SFPE Handbook~\cite{SFPE:Tewarson}. A balanced chemical reaction for wood combustion can written as:
