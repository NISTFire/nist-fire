...
Since the time at which the fire started is not known relative to the notification of a fire or how fast the fire spread, several simplifying assumptions were made. One assumption is that the initial couch fire in the basement was specified to start at the beginning of the simulation. The beginning of the simulation ($t=0$) was selected to correspond to an on-scene time of 10:53:00 (see Table~\ref{tab:incident_timeline}). The HRRs for the three primary source fires was prescribed in FDS as follows.

\begin{enumerate}
\item For the initial couch fire in the basement, the HRR from 0~s to 327~s followed the experimentally measured mattress HRR. At 300~s, the HRR was approximately 3~MW. From 327~s until 540~s (the end of the simulation), the HRR was a constant 5~MW.
