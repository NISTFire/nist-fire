...
Data acquisition systems are constructed using National Instruments (NI) hardware. The systems starts with the chassis - NI SCXI-1000/NI SCXI-1001. The 1000 is a 4-slot chassis while the 1001 is a 12-slot chassis. The max scan rate of the chassis is (5 $\mu$s $\times$ \# of channels used)$^{-1}$. Connecting the chassis to the computer (via USB) is the NI SCXI-1600 which is a 200 kS/s 16-bit digitizer.
...
\midrule
Bare bead  &  & \\ 
Inconel sheathed    & 0 - 1260 $^{\circ}$C &  MAX($\pm$ 1.11$^{\circ}$C; $\pm$ 0.4 $\%$)   \\ 
\bottomrule[1.25pt]
\end{tabular}\par
\end{table}
...
Two different welders are used to construct the bare bead TCs.
