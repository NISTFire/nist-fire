...
\subsection{Attic Fires}

The fuel load for the attic fires, experiments 9 and 10, was essentially the OSB roof underlayment, and the wood trusses that support the roof.  A 1.2 m (4 ft) square hole was opened between the center of the southern room in the single story structure and the attic space. A single stack of pallets as described for the wood fuel package tests was loacted below the hole and ignited.  The fire was allowed to spread into the attic space.  In addition to the connecting hole between the room and the attic space, the attic had openings along the eave line on both sides of the structure and gable openings on each end of the attic space.    


\subsubsection*{Two Story Structure}
\label{sec:suppresion_two}

In June of 2015, the larger compartment experiments, experiments 15 through 22, were conducted in the lower level of the two story structure.  Two different fuel packages were utitlized, the difference between the furniture fuel package used for experiments 15 and 16, and the furnitiure and pallet fuel package used in experiments 17 through 22 was the addition of two stacks of pallets.   
...
To better quantify the effectiveness of the suppression agents (CAF, water), the maximum temperature differences associated with each gas cooling event for each agent from the three thermocouple trees were analyzed. Figure~\ref{fig:combined_all} compares the temperature differences at each thermocouple position (via a scatterplot) for the 30 replicate tests, 15 water and 15 CAF, for all streams aimed at all positions.
...
\end{figure}
Based on the monitor position (arrow at the doorway in Fig.~\ref{fig:Gas_Cooling_Instrumentation_Dimensions2}), thermocouple arrays A1 and A2 had more direct impact from the suppression agent compared to thermocouple array A3. Figure~\ref{fig:Burn_Building_Test_18b} shows the distribution of the agent within the burn building from a spray density test for a straight stream with CAF at the mid position compared to the positions of the thermocouple trees.
\begin{figure}[!ht]
...
The thermocouple array data can also also be decomposed as a function of nozzle position: 12 middle position replicates and 3 back position replicates. Figures~\ref{fig:CAFS_Water_A1_mid}, \ref{fig:CAFS_Water_A2_mid}, and \ref{fig:CAFS_Water_A3_mid} show the temperature difference scatter at thermocouple arrays A1, A2, and A3 respectively for the 12 replicate tests with the monitor nozzle in the middle position. Figures~\ref{fig:CAFS_Water_A1_back}, \ref{fig:CAFS_Water_A2_back}, and \ref{fig:CAFS_Water_A3_back} show the temperature difference scatter at thermocouple arrays A1, A2, and A3 respectively for the 3 replicate tests with the monitor nozzle in the back position.
...
Test $\#$  & Name	& Stream			& Flow Rate		& Fuel                    & Ventilation  \\
\midrule
 38  & CAF~7     &  Straight Stream  	&  120~gpm/60~cfm   & Furniture           & Basement Doors, 1st Floor Door \\
 39  & Water~7   &  Straight Stream  	&  120~gpm    		& Furniture           & Basement Doors, 1st Floor Door \\
 40a & CAF~8a    &  Straight Stream  	&  120~gpm/60~cfm   & Furniture + Pallet  & Basement Doors, 1st Floor Door \\
 40b & CAF~8b    &  Straight Stream  	&  120~gpm/60~cfm   & Furniture + Pallet  & Basement Doors, 1st Floor Door \\
 40c & Water~8   &  Straight Stream     &  120~gpm          & Furniture + Pallet  & Basement Doors, 1st Floor Door \\
 41a & Class A~1 &  Straight Stream  	&  120~gpm		    & Furniture + Pallet  & Basement Doors, 1st Floor Door \\
 41b & CAF~9a    &  1 3/8'' Ball    	&  120~gpm		    & Furniture + Pallet  & Basement Doors, 1st Floor Door \\
 41c & CAF~9b    &  Solid Stream        &  120~gpm		    & Furniture + Pallet  & Basement Doors, 1st Floor Door \\
\bottomrule[1.25pt]
\end{tabular}\par
\end{table}
...
In each gas cooling experiment, the same mass flow rate of water was used for both the plain water stream and the CAF stream. Typically the volumetric flow rate of water was 7.57~L/s (120~gpm). The density of water is 1000~kg/m$^3$ (8.3~lb/gal), therefore the mass flow rate was 7.57~kg/s (16.7~lb/s). The average time of flow was 15~s, therefore a total of 114~kg (251~lb) of water was introduced into each gas cooling experiment. {\bf When a CAF stream was used it contained 0.3\% foam concentration by volume, check density of foam concentrate   point being mostly water.  Also calc the mass of air in the stream.}

Under the conditions of a residential scale room and contents fire it should be no surprise that the results are similar, the mass of water introduced into the hot gas layer is doing the work. Air filled bubbles in the CAFS stream had no discernible impact in these experiments.
...
The Iowa Formula for determining the required fire flow for a given structure was developed, by Keith Royer and Bill Nelson, based on theoretical heat absorption capabilities of water. The required fire flow in gpm is equal to the volume of the fire compartment (in cubic feet) divided by 100:
\begin{equation}\label{eq:isu_form}
\dot{Q} = V / 100
\end{equation}
Due to inefficiencies in applying the water, some feel that the fire flow value from the Iowa State University formula should be multiplied by a factor of two to four~\cite{NFPA}. Applying the Iowa State University formula based on floor area yields an application density of 3.3~Lpm/m$^2$ (0.08~gpm/ft$^2$) for ceiling heights of 2.44~m (8~ft) or 4.1~Lpm/m$^2$ (0.10~gpm/ft$^2$) for ceiling heights of 3.05~m (10~ft). 
...
\dot{Q} = A / 3
\end{equation}
Applying the NFA formula based on floor area yields an application density of 13.6~Lpm/m$^2$ (0.33~gpm/ft$^2$).
