...
The 88 gas cooling experiments were completed as part of 9 separate tests in the burn building. During each test, the fire was ignited and the fire developed until the temperature 1.83~m (6~ft) below the ceiling in the gas cooling room reached or exceeded 260~$^{\circ}$C (500~$^{\circ}$F). Once this criteria was met, gas cooling began. 
...
\begin{figure}[!ht]
	...
	\caption{CAFS vs Water Gas Cooling Comparison for Straight Stream Full Back Position}
	\label{fig:CAFS_Water_Comp_SS_full}
\end{figure}
...
Figures~\ref{fig:Test_1_First_Suppression} and \ref{fig:Test_1_Second_Suppression} provide and overview of the impact of 30$^{\circ}$ fog CAF application on the furniture fuel package after 15~s of application from the hallway nozzle and then 15~s of application from the room nozzle (FSE1 in Table~\ref{tab:Test_Descriptions}). The figures provide change in temperature and heat flux measurements at several positions and elevations through the structure: structure doorway, hallway, and three locations within the burn room. These values are taken immediately before and after application to assess the application agent, stream, and location on interior conditions.
...
The same fuel load, ventilation, and suppression application durations were also tested with water as the suppression agent (FSW2 in Table~\ref{tab:Test_Descriptions}). Figures~\ref{fig:Test_2_First_Suppression} and \ref{fig:Test_2_Second_Suppression} show the results from these tests.
...
Note that while there are some differences in the changes in temperature and heat flux when comparing CAFs to Water, differences are not substantial. Both suppression agents had similar, significant impact in reducing the high-temperature, high-heat flux environment that existed before suppression. The remainder of these fire suppression images for both the furniture and wood pallet fuel loads can be found in Appendix~\ref{app:fire_suppression}.
