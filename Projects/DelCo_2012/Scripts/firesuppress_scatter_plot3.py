...
del_TC_3_h2o_hall = np.zeros((7,6))
del_TC_3_cafs_hall = np.zeros((7,6))
del_TC_3_h2o_door = np.zeros((7,6))
del_TC_3_cafs_door = np.zeros((7,6))
            ...
            if test_name in info2['Test_Series_Water'][reps]:
                for group in channel_groups:
                    ...
                    for channel in data.columns[1:]:
                        ...
                        if 'FSE3_' in channel or 'FSW3_' in channel:
                            if 'HF2_' in channel:
                                continue
                            if 'HF_' in channel:
                                continue
                            ...
                            del_TC_3_h2o_hall[ii,reps-1] =  max(water_data_interval) - max(min(water_data_interval),0)
                            del_TC_3_h2o_door[ii,reps-1] =  max(water_data_interval2) - max(min(water_data_interval2),0)
                            ii=ii+1
                ...
                for group in channel_groups:
                    ...
                    for channel in data.columns[1:]:
                        ...
                        if 'FSE3_' in channel or 'FSW3_' in channel:
                            ...
                            del_TC_3_cafs_hall[ii,reps-1] =  max(max(cafs_data_interval),0) - max(min(cafs_data_interval),0.)
                            del_TC_3_cafs_door[ii,reps-1] =  max(max(cafs_data_interval2),0) - max(min(cafs_data_interval2),0.)
                            ii=ii+1
del_water = concatenate([del_TC_3_h2o_hall,del_TC_3_h2o_door], axis=1)
del_cafs = concatenate([del_TC_3_cafs_hall,del_TC_3_cafs_door], axis=1)
...
savefig(save_dir + 'TCA3_hallnozzle_scatter.pdf',format='pdf')



del_water = del_TC_3_h2o_door
del_cafs = del_TC_3_cafs_door
del_water = del_water.reshape(-1)
del_cafs  = del_cafs.reshape(-1)
dif_data  = []
for i in range (0,len(del_water)):
...
xlabel('$\Delta$T ($^{\circ}$C) CAFS')
ylabel('$\Delta$T ($^{\circ}$C) WATER')
savefig(save_dir + 'TCA3_doornozzle_scatter.pdf',format='pdf')