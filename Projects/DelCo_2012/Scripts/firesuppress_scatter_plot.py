...
del_TC_1_h2o_hall = np.zeros((8,6))
del_TC_1_cafs_hall = np.zeros((8,6))
del_TC_1_h2o_door= np.zeros((8,6))
del_TC_1_cafs_door = np.zeros((8,6))
				...
				for group in channel_groups:
					...
					for channel in data.columns[1:]:
						...
						if 'FSE1_' in channel or 'FSW1_' in channel:
							...
							del_TC_1_h2o_hall[ii,reps-1] =  max(water_data_interval) - max(min(water_data_interval),0)
							del_TC_1_h2o_door[ii,reps-1] =  max(water_data_interval2) - max(min(water_data_interval2),0)
							ii=ii+1
...
del_water = del_TC_1_h2o_hall
del_cafs = del_TC_1_cafs_hall
...
savefig(save_dir + 'TCA1_hallnozzle_scatter.pdf',format='pdf')

# print(del_TC_1_h2o_door)
# print(del_TC_1_cafs_door[:,4])

del_water = del_TC_1_h2o_door
del_cafs = del_TC_1_cafs_door
del_water = del_water.reshape(-1)
del_cafs  = del_cafs.reshape(-1)
dif_data  = []
for i in range (0,len(del_water)):
...
xlabel('$\Delta$T ($^{\circ}$C) CAFS')
ylabel('$\Delta$T ($^{\circ}$C) WATER')
savefig(save_dir + 'TCA1_doornozzle_scatter.pdf',format='pdf')