...
del_TC_1_h2o = np.zeros((8,5))
del_TC_1_cafs = np.zeros((8,5))
del_TC_2_h2o = np.zeros((8,5))
del_TC_2_cafs = np.zeros((8,5))
...
for f in os.listdir(data_dir):
    if f.endswith('.csv'):
        ...
        channel_groups = [['FSE1_'],['FSE2_'],['FSW1_'],['FSW2_']]
        ...
        for reps in range(1,len(info2)):
        	if test_name in info2['Test_Series_Water'][reps]:
	        	for group in channel_groups:
        			if 'FSE1_' or 'FSW1_' in group:
        				room = 'Hallway'
        			else:
        				room='Door'
        			ii=0
        			...
        			for channel in data.columns[1:]:
        				...
        				if 'FSE1_' in channel or 'FSW1_' in channel:
        					if 'HF_' in channel:
        						continue
        					if 'RAD_' in channel:
        						continue
	        				water_data_interval = current_channel_data[info2[room+'_Start_Time_Water'][reps]:info2[room+'_Stop_Time_Water'][reps]]
        					del_TC_1_h2o[ii,reps-1] =  max(water_data_interval) - min(water_data_interval)
        					ii=ii+1
        				elif 'FSE2_' in channel or 'FSW2_' in channel:
        					if 'HF_' in channel:
        						continue
        					if 'RAD_' in channel:
        						continue
	        				water_data_interval = current_channel_data[info2[room+'_Start_Time_Water'][reps]:info2[room+'_Stop_Time_Water'][reps]]
        					del_TC_2_h2o[kk,reps-1] =  max(water_data_interval) - min(water_data_interval)
        					kk = kk + 1
	        	...
	        	for group in channel_groups:
        			...
        			for channel in data.columns[1:]:
        				...
        				if 'FSE1_' in channel or 'FSW1_' in channel:
        					...
        					if 'RAD_' in channel:
        						continue
	        				cafs_data_interval = current_channel_data[info2[room+'_Start_Time_CAFS'][reps]:info2[room+'_Stop_Time_CAFS'][reps]]
        					del_TC_1_cafs[ii,reps-1] =  max(cafs_data_interval) - min(cafs_data_interval)
        					ii=ii+1
        				elif 'FSE2_' in channel or 'FSW2_' in channel:
        					if 'HF_' in channel:
        						continue
        					if 'RAD_' in channel:
        						continue
	        				cafs_data_interval = current_channel_data[info2[room+'_Start_Time_CAFS'][reps]:info2[room+'_Stop_Time_CAFS'][reps]]
        					del_TC_2_cafs[kk,reps-1] =  max(cafs_data_interval) - min(cafs_data_interval)
        					kk = kk + 1
print(del_TC_2_h2o)
print(del_TC_2_cafs)

toggle_plot = 1
del_water = del_TC_1_h2o
del_cafs = del_TC_1_cafs
...
savefig(save_dir + loc + '_scatter.pdf',format='pdf')