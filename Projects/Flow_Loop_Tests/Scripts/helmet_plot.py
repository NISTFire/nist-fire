...
# Location of files
data_dir = '../Experimental_Data_Helmets/'
plot_dir = '../Figures/Helmets/'
info_file = '../Experimental_Data_Helmets/Description_of_Experiments.csv'
...
for f in os.listdir(data_dir):
	if f.endswith('.csv'):
		...
		fig = figure()
		plot(data['Time'],data['TC17'],'b*-',markevery=50,ms=8,label='Air Temperature')
		plot(data['Time'],data['TC16'],'rs-',markevery=50,ms=8,label='Front of Helmet')
		plot(data['Time'],data['TC18'],'g^-',markevery=50,ms=8,label='Inside of Helmet')
		plot(data['Time'],data['TC19'],'co-',markevery=50,ms=8,label='Buckle')
		plot(data['Time'],data['TC20'],'m<-',markevery=50,ms=8,label='Headform')
		ax1 = gca()
