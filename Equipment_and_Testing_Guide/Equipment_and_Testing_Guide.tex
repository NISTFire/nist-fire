...
\item Create .config file 
    \begin{itemize}
    \item Open \textit{rabbitmq.config.example} (./etc/rabbitmq/ on Mac/Linux; ./etc/ on Windows)
    \item Add `[ \{loopback\_users, [] \}' after the `\{rabbit,' line and save file as `rabbitmq.config' in the directory containing the example file \\
    \textit{Note:} On Windows, may have to save it in the directory \\
    `C:\textbackslash Users/\%USERNAME\%/AppData/Roaming/RabbitMQ/' \\
    To find such directory, search `rabbitmq.config.example' on computer and open approriate file location.
    \end{itemize}
